"""
복어(Fugu) 오케스트레이션 — 라우팅 → 페르소나 생성 → KPI/과금 종합.

중앙집중 멀티에이전트(매니저+작업에이전트) = agentic-RAG 구조.
비용은 게이트 라우팅(필요한 페르소나만) + 모델 티어링으로 통제하고,
모든 호출을 계측해 실사용 원가 기준으로 포인트 차감한다(계측=과금, §6·§7).
"""
from . import personas, router
from .llm import Meter, get_provider


def run_geo(merchant, question: str = "", provider=None, meter=None) -> dict:
    """업체 베이스라인을 받아 복어(Fugu)가 종합 액션 + 실사용 원가를 산출."""
    ...
    return {
        ...
        "billing": {
            ...
            # 포인트 차감(points_charged/balance)은 뷰가 Wallet과 함께 채운다
            "raw_cost_usd": round(meter.total_cost_usd(), 6),
            "calls": meter.breakdown(),
            "note": "계측=과금: 실사용 토큰 원가만큼 포인트 차감. provider=stub면 무과금.",
        },
